...
output_list <- list(results = x,
                    positive = nrow(x[x$p_gt < 0.05,]),
                    negative = nrow(x[x$p_lt < 0.05,]),
                    co_occurrences = (nrow(x[x$p_gt < 0.05 | x$p_lt < 0.05,])), #nrow(output[output$p_exactly_obs <= 0.05,]),
                    pairs = nrow(x),
                    species = 20,
                    spp_key = zz,
